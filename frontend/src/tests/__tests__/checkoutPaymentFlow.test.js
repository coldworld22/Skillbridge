...
jest.mock('@/services/auth/authService', () => ({
  refreshAccessToken: jest.fn().mockResolvedValue({ accessToken: 'test-token' }),
}));
...
test('adjusts inputs based on payment selection and submits bank reference', async () => {
  const push = jest.fn();
  mockUseRouter.mockReturnValue(createRouter({ push }));
  fetchPaymentMethods.mockResolvedValue([
    { id: 2, name: 'Pay Pal', type: 'manual' },
    {
      ...
      name: 'Bank',
      type: 'bank',
      settings: { bank_name: 'Test Bank', account_holder_name: 'John', account_number: '123', swift_code: 'ABCDEF' },
    },
  ]);
  ...
  expect(
    screen.getByRole('button', { name: /Pay \$100\.00 with PayPal/i })
  ).toBeInTheDocument();
  expect(
  ...
  fireEvent.click(screen.getByRole('button', { name: /Pay \$100\.00 with Bank/i }));
...
test('shows error when PayPal payment lacks approval_url', async () => {
  ...
  render(<CheckoutPage />);
  await screen.findByText('Checkout');
  const button = screen.getByRole('button', { name: /Pay \$100\.00 with PayPal/i });
...
test('shows error when crypto payment lacks invoice_url', async () => {
  ...
  render(<CheckoutPage />);
  await screen.findByText('Checkout');
  const button = screen.getByRole('button', { name: /Pay \$100\.00 with Crypto/i });
  fireEvent.click(button);
...
test('shows available payment methods for plans', async () => {
  mockUseRouter.mockReturnValue(
    createRouter({ query: { itemId: '1', itemType: 'plan' } })
  );
