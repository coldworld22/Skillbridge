...
test('coinbase payment detection falls back to name when type mismatches', async () => {
  initiateCoinbasePayment.mockResolvedValue({ hosted_url: 'http://coinbase' });
  const args = {
    ...baseArgs(),
    method: { type: 'paypal', name: 'Coinbase Commerce' },
  };
  await handleCryptoPayment(args);
  expect(initiateCoinbasePayment).toHaveBeenCalledWith(
    expect.objectContaining({ method_type: 'coinbase' })
  );
  expect(window.location.href).toBe('http://coinbase');
});
