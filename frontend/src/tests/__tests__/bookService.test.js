...
describe("bookService", () => {
  ...
  it("omits empty filter defaults when fetching books", async () => {
    ...
    expect(api.get).toHaveBeenCalledWith("/books", {
      params: { status: "active", priceRange: 0 },
    });
  });
  ...
  it("fetches single book as instructor", async () => {
    const apiData = { id: 2, title: "B" };
    api.get.mockResolvedValueOnce({ data: { data: apiData } });
    const book = await fetchBook(2, { instructor: true });
    expect(api.get).toHaveBeenCalledWith("/instructor/books/2");
    expect(book).toMatchObject({
      id: 2,
      title: "B",
    });
  ...
  it("falls back to public endpoint if instructor fetch fails", async () => {
    const apiData = { id: 2, title: "B" };
    api.get
      .mockRejectedValueOnce(new Error("fail"))
      .mockResolvedValueOnce({ data: { data: apiData } });
    const book = await fetchBook(2, { instructor: true });
    expect(api.get).toHaveBeenNthCalledWith(1, "/instructor/books/2");
    expect(api.get).toHaveBeenNthCalledWith(2, "/books/2");
    expect(book).toMatchObject({
      id: 2,
      title: "B",
    });
  ...
  it("buildUrl rewrites blocked internal hosts to relative API paths", () => {
    const originalBase = process.env.NEXT_PUBLIC_API_BASE_URL;
    jest.isolateModules(() => {
      process.env.NEXT_PUBLIC_API_BASE_URL = "http://backend:5002/api";
      const { buildUrl } = require("../../services/bookService");
      expect(buildUrl("/uploads/test.jpg")).toBe("/api/uploads/test.jpg");
    });
    process.env.NEXT_PUBLIC_API_BASE_URL = originalBase;
