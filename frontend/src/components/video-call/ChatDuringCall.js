...
import styles from "./ChatDuringCall.module.scss";
  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.inputRow}>
        ...
        />
        <button className={styles.send} onClick={sendMessage} type="button">
          <FaPaperPlane />
        </button>
      </div>
    </div>
  ...
  return (
    <div className={bubbleClasses.join(" ")}>
      <div className={styles.header}>
        ...
        {flagged && (
          <span
            className={`${styles.badge} ${severityBadgeClasses(
              severity
            )}`}
          >
      ...
      <p
        className={`${
          flagged && blocked && !isHost && !isSelf ? styles.mutedText : ""
        }`}
      >
        {displayText}
      </p>
      {isHost && flagged && msg.flag_metadata?.reason && (
        <p className={styles.flagInfo}>
