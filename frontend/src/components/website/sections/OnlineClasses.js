...
const BUILTIN_CATEGORY_ALL = "__ALL__";
const BUILTIN_CATEGORY_TRENDING = "__TRENDING__";
const builtInCategoryValues = [BUILTIN_CATEGORY_ALL, BUILTIN_CATEGORY_TRENDING];
const excludedBackendCategoryLabels = new Set(["All", "Trending"]);
...
const OnlineClasses = () => {
  const [selectedCategory, setSelectedCategory] = useState(BUILTIN_CATEGORY_ALL);
  ...
  const [categories, setCategories] = useState(builtInCategoryValues);
      ...
      try {
        ...
        const cats = Array.from(
          new Set(
            formatted
              .map((c) => c.category)
              .filter((cat) => cat && !excludedBackendCategoryLabels.has(cat))
          )
        );
        setCategories([...builtInCategoryValues, ...cats]);
      } catch (err) {
  ...
  const filteredClasses = useMemo(() => {
    ...
    const filtered = classes.filter((classItem) => {
      const matchesCategory =
        selectedCategory === BUILTIN_CATEGORY_ALL ||
        (selectedCategory === BUILTIN_CATEGORY_TRENDING && classItem.trending) ||
  ...
  const handleResetFilters = () => {
    setSelectedCategory(BUILTIN_CATEGORY_ALL);
          ...
          <div className="flex flex-wrap justify-center items-center gap-4 mb-8">
            ...
            >
              {categories.map((category) => {
                let label = category;
                if (category === BUILTIN_CATEGORY_ALL) {
                  label = t("online_classes_filter_all");
                } else if (category === BUILTIN_CATEGORY_TRENDING) {
                  label = t("online_classes_filter_trending");
                }
                return (
