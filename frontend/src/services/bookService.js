...
export const formatBook = (book) => {
  ...
  const coverCandidatesRaw = collectPaths(
    book?.coverUrl,
    book?.cover_url,
  ...
  let rawCoverPath = coverCandidatesRaw.find(Boolean);
...
export const fetchBook = async (
  id,
  { admin = false, instructor = false, ...config } = {}
) => {
  ...
  try {
    ...
    if (!admin && !instructor && isNotFound) return null;

    if ((admin || instructor) && err.name !== "CanceledError" && err.name !== "AbortError") {
      try {
        const { data } = hasConfig
