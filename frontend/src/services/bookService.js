...
export const formatBook = (book) => {
  ...
  const extractPath = (value) => {
    ...
    if (typeof value === "object") {
      return (
        ...
        extractPath(value.original_url) ||
        extractPath(value.thumbnail_url) ||
        extractPath(value.preview_url) ||
        extractPath(value.secure_url) ||
        null
      );
    }
    return null;
  ...
  const coverCandidates = collectPaths(
    book?.coverUrl,
    book?.cover_url,
  ...
  const rawCoverPath = coverCandidates.find(Boolean);
