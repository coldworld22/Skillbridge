...
function MyEnrolledClassesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!active) return;
        setClasses(list);
        const seededStatus = list.reduce((acc, item) => {
          if (!item?.id) return acc;
          acc[item.id] = item.reminderSubscribed ? 'success' : 'idle';
          return acc;
        }, {});
        setReminderStatus(seededStatus);
      } catch (err) {
  ...
  const handleReminderSubscribe = async (classId) => {
    try {
      ...
      setClasses((prev) =>
        prev.map((item) =>
          String(item?.id ?? item?.linkId ?? '') === normalizedId
        ...
        {visibleClasses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {visibleClasses.map(cls => {
              ...
              const reminderSubscribed = cls.reminderSubscribed || reminderState === 'success';
              ...
              return (
                ...
                <div className="flex justify-between items-start">
                  ...
                  <FaEye
                    ...
                    tabIndex={0}
                    onClick={() => {
                      const target = cls.id || cls.linkId;
                      if (!target) return;
                      router.push(`/dashboard/student/online-classes/${target}`);
                    }}
                    onKeyDown={(event) => {
                ...
                {normalizedSchedule === 'ongoing' ? (
                  <Link
                    href={`/dashboard/student/online-classes/${cls.id || cls.linkId}`}
                    className="text-xs text-green-700 underline mb-3 block text-center font-semibold"
                  >
                    <FaVideo className="inline mr-1" /> {t('cta_open_classroom')}
                  </Link>
                ) : normalizedSchedule === 'upcoming' ? (
...
const ProtectedStudentClasses = withAuthProtection(MyEnrolledClassesPage, ['student']);
...
export default ProtectedStudentClasses;
