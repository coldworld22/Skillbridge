...
export default function StudentSettingsPage() {
  ...
  const handleDownloadInvoice = useCallback(
    async (payment, existingInvoice = null) => {
      if (!payment) return;
      const mergedInvoice =
        existingInvoice ||
        (payment?.id ? invoicesByPayment.get(payment.id) : null);
      let invoiceId = mergedInvoice?.id || payment?.invoice_id || null;
      let invoicePdfUrl =
        mergedInvoice?.pdf_url || payment?.invoice_pdf_url || null;

      try {
        if (!invoiceId && !invoicePdfUrl) {
        ...
        if (invoiceId) {
          await downloadStudentInvoice(invoiceId);
          return;
        }
        if (invoicePdfUrl) {
          window.open(invoicePdfUrl, "_blank", "noopener");
          return;
        }

        toast.info(t("messages.invoice_unavailable"));
      } catch (err) {
                              ...
                              {invoiceAvailable ? (
                                <>
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleDownloadInvoice(payment, mergedInvoice)
                                    }
                                    className="inline-flex items-center gap-2 text-sm font-semibold text-yellow-600 hover:text-yellow-700"
                                  >
                                    <FaDownload />
                                ...
                                <span className="text-xs text-gray-400">
                                  {t("messages.invoice_unavailable")}
                                </span>
                              )}
                            </td>
                          </tr>
