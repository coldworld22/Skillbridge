...
const normalizeProgress = (value) => {
  if (value && typeof value === "object") {
    if (typeof value.percent !== "undefined") {
      return normalizeProgress(value.percent);
    }
    if (typeof value.percentage !== "undefined") {
      return normalizeProgress(value.percentage);
    }
    if (typeof value.value !== "undefined") {
      return normalizeProgress(value.value);
    }
  }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) setClasses(formatted);
      } catch (err) {
