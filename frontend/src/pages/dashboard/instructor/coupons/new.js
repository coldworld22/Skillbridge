...
const generateCode = () =>
  `SB-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
...
export default function InstructorCouponNew() {
  ...
  useEffect(() => {
    ...
    const loadTargets = async () => {
      ...
      try {
        const data = await fetchCouponTargets();
        if (!active) return;
        setTargets(data);
      } catch (_err) {
        if (!active) return;
        ...
        <div className="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-white/70 p-6 shadow-sm">
          <header className="mb-6">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Reward loyal students or promote a launch with a targeted offer.
            </p>
          </header>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-1">
                ...
                <div className="flex rounded-md border border-slate-200 shadow-inner focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-200">
                  <input
                    {...register("code")}
                    placeholder="e.g. LAUNCH25"
                    className="w-full rounded-md border-0 bg-transparent px-3 py-2 text-sm uppercase tracking-wide text-slate-900 outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => setValue("code", generateCode(), { shouldValidate: true })}
                    className="border-l border-slate-200 px-3 text-xs font-semibold uppercase text-emerald-600 transition hover:bg-emerald-50 hover:text-emerald-700"
                  >
                    Randomize
                  </button>
                </div>
                ...
                <p className="text-xs text-slate-500">
                  Codes are not case sensitive. We recommend short memorable
                  names.
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500">
                  Percentage discount applied to the selected item price.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                type="submit"
                disabled={isSubmitting || loadingTargets || !scopedItems.length}
