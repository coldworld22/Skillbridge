...
export default function InstructorCommunityPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return discussions.filter((d) => {
      ...
      if (activeTag) {
        return d.tags.some((tag) => tag.toLowerCase().includes(activeTag.toLowerCase()));
      }
  ...
  const stats = useMemo(
    () => [
      ...
      { label: "Your Threads", value: myQuestionsCount },
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-10">
          <section className="overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white shadow-2xl">
            <div className="flex flex-col gap-8 p-8 md:flex-row md:items-center md:justify-between">
              <div className="max-w-2xl space-y-4">
                ...
                <h1 className="text-3xl font-bold md:text-4xl">Guide discussions and mentor your cohorts</h1>
                <p className="text-sm text-slate-200/80">
                  Surface important threads, highlight expert answers, and keep the conversation flowing between classes.
                </p>
              </div>
              <Link href="/dashboard/instructor/community/ask" className="shrink-0">
          ...
          <section className="grid gap-6 lg:grid-cols-[280px,1fr]">
            <aside className="space-y-6 rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm">
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-widest text-slate-500" htmlFor="instructor-community-search">
                  Search
                </label>
                <div className="relative mt-2">
                  <input
                    id="instructor-community-search"
                    type="text"
            ...
            <div className="space-y-4">
              {paginated.length > 0 ? (
                paginated.map((d) => (
                  <Link key={d.id} href={`/dashboard/instructor/community/questions/${d.id}`}>
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 bg-white/80 p-10 text-center text-slate-500">
                  ...
                  <p className="mt-1 text-sm">Refine your search or create a new prompt to kick things off.</p>
                </div>
              )}
