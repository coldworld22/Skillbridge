...
function InstructorBookDetailPage() {
  ...
  const tags = useMemo(() => {
    ...
    if (Array.isArray(book.tags)) {
      return book.tags.map(normalizeTag).filter(Boolean);
    }
  ...
  const coverSource = useMemo(() => {
    ...
    const resolved = candidates
      .map((candidate) => buildUrl(candidate) || candidate)
      .find(Boolean);
    return resolved || "/images/default-book-cover.jpg";
  }, [
    book?.coverUrl,
    book?.cover_image_url,
    book?.cover_image,
  ...
  const [coverSrc, setCoverSrc] = useState(coverSource);

  useEffect(() => {
    setCoverSrc(coverSource);
  }, [coverSource]);
  ...
  return {
    props: {
      ...(await serverSideTranslations(
        locale,
        ["dashboard", "errors", "website", "common"],
        nextI18NextConfig
      )),
    },
  };
