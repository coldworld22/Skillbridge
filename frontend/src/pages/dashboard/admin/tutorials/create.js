...
function CreateTutorialPage() {
  ...
  const submitTutorial = async (status) => {
    const targetStatus = status || tutorialData.status || "draft";
    const isPublishing = targetStatus === "published";
    if (isPublishing && tutorialData.chapters.some((ch) => !ch.videoUrl)) {
    ...
    const formData = buildTutorialFormData(tutorialData, targetStatus);

    try {
      await createTutorial(formData);
      toast.success(
        targetStatus === "draft" ? t('draft_saved') : t('submitted_success')
      );
      const msg =
        targetStatus === "draft"
