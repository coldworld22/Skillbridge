...
> **After setup:** Run these quick checks to confirm the Docker stack and
> database are ready for day-to-day use.

- Visit `http://localhost:3000` (or your domain) to confirm the Next.js
  frontend loads without console errors.
- Sign in with the admin account created during `install.sh`, then open
  `/dashboard/admin` to verify you can reach the admin panel.
- Hit the API health endpoint at `http://localhost:5002/api/health`
  (or `https://<domain>/api/health`) and ensure it reports `status: ok`.
- Check the backend logs with `docker compose logs backend` for migration errors
  or SMTP failures before sharing the environment.
...
Set `INSTALL_PURCHASE_CODE` (or `INSTALL_SETUP_SECRET`) to the passphrase you want to require. The default packaged value is `Javaheat@18880`. Every request to `/api/install/*` must send the same value via the `X-Install-Purchase-Code` or `X-Install-Setup-Secret` header. When `INSTALL_REQUIRE_AUTH=true` you must also include an administrator JWT (`Authorization: Bearer …`). With auth disabled (the default), the purchase code alone unlocks the API and mirrors the behavior of the browser wizard in Step&nbsp;2.

```http
POST /api/install/run
Content-Type: application/json
X-Install-Purchase-Code: Javaheat@18880
# Optional when INSTALL_REQUIRE_AUTH=true
...
If you configure `INSTALL_SETUP_SECRET` in `backend/.env`, clients can provide the value via `X-Install-Setup-Secret` instead of `X-Install-Purchase-Code`. The backend trims the configured secret before comparison, so avoid trailing spaces when setting the environment variable. Requests that omit the header or provide the wrong secret receive a `403` response with an `INSTALL_LOCKED` error code before the installer runs.
