...
- `FRONTEND_MAX_OLD_SPACE_SIZE` (MB, default `6144`) caps the Node heap size used by `npm run build`. Lower the value on very small hosts if the OS kills the build.
- `FRONTEND_DISABLE_MINIFY` now defaults to `auto`. When running inside a container with less than ~6 GiB of RAM, the build script automatically sets this to `1` to avoid `SIGKILL`s from the kernel. Set `FRONTEND_DISABLE_MINIFY=0` to force minification or `1` to always skip it.
- `FRONTEND_MINIFY_MEMORY_THRESHOLD_GB` (default `6`) controls the cut-off used by the auto-detection logic above.
