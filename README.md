...
Alternatively, launch the backend and open [`/install`](http://localhost:5002/install) to use a simple web-based installer that checks prerequisites and runs the setup scripts after entering configuration values. Enable it temporarily by setting `ENABLE_INSTALL=true` **and** `INSTALL_API_ENABLED=true` inside `backend/.env`, then restart the backend. Keep `INSTALL_REQUIRE_AUTH=false` if you want to skip admin login and share the purchase code (`INSTALL_PURCHASE_CODE`, default `Javaheat@18880`) with whoever is running the wizard. Disable the installer flags again once setup is complete.
...
2. Initialize the database:
   ...
   2. Seed development data:
      ...
      `backend/src/seeds`. Only the SuperAdmin and (optional) Admin demo users
      are seeded now; all catalog data (plans, categories, etc.) must be added
      through the dashboard after signing in. A raw SQL snapshot of the schema
      is also available at `database/schema.sql` for reference.
