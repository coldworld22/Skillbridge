...
router.get('/:roomId/status', verifyToken, verifyVideoCallAccess, async (req, res) => {
  await ensureVideoCallSchema();
  const { roomId } = req.params;
  try {
    const [hostRow] = await db('video_call_participants')
      .where({ room_id: roomId })
      .andWhere('left_at', null)
      .andWhere('role', 'host')
      .count('id as count');
    const [participantRow] = await db('video_call_participants')
      .where({ room_id: roomId })
      .andWhere('left_at', null)
      .count('id as count');

    const session = state.sessions[roomId] || null;
    const live = Number(hostRow?.count || 0) > 0 || Boolean(session?.live);

    res.json({
      roomId,
      live,
      hostCount: Number(hostRow?.count || 0),
      activeParticipants: Number(participantRow?.count || 0),
      session: session
        ? {
        ...
        : { live, startedAt: null, endedAt: null },
      reportedAt: new Date().toISOString(),
    });
  } catch (err) {
    logger.error('Failed to fetch live session status', err);
