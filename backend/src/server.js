...
const resolveStaticDirectory = (label, candidates = [], options = {}) => {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    try {
      ...
      if (strict && index === 0) {
        logger.warn?.(`${label} override is not a directory`, {
          path: absolute,
        });
        return null;
      }
...
app.use(async (req, res, next) => {
  ...
  const clientIp =
    ...
    "unknown";
  const requestMeta = `${req.method} ${req.originalUrl || req.url}`;
    ...
    if (!allowedHostSet.has(hostname)) {
      try {
        ...
        if (!custom) {
          ...
          metrics.increment("forbidden_host", {
            host: hostname,
            ip: clientIp,
          });
  ...
  if (originHeader) {
    if (!allowedOriginSet.has(originHeader)) {
      logger.warn(
        `Rejected request with invalid origin header: ${originHeader} (ip=${clientIp} request=${requestMeta})`,
      );
    ...
    try {
      ...
      if (!allowedOriginSet.has(refererOrigin)) {
        logger.warn(
          `Rejected request with invalid referer header: ${req.headers.referer} (ip=${clientIp} request=${requestMeta})`,
        );
        return res.status(403).json({ message: "Referer not allowed" });
      }
    } catch (error) {
      logger.warn(
        `Failed to parse referer header: ${req.headers.referer} (ip=${clientIp} request=${requestMeta})`,
      );
  ...
  const sendInstaller = (req, res) => {
    ...
    if (relativePath) {
      ...
      if (
        ...
        logger.warn?.(
          "Blocked attempt to access installer asset outside root",
          {
            requested: candidate,
          },
        );
