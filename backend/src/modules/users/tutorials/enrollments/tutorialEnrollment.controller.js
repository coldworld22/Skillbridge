...
const parseIncludedPlans = (value) => {
  ...
  if (typeof value === "string") {
    try {
      ...
      return Array.isArray(parsed)
        ? parsed.map((id) => `${id}`.trim()).filter(Boolean)
        : [];
    } catch (_) {
      return [];
    }
...
exports.enroll = catchAsync(async (req, res) => {
  ...
  const includedPlans = parseIncludedPlans(tutorial.included_plans);
  const coveredBySubscription =
    activePlanId &&
    includedPlans
      .map((id) => `${id}`)
      .includes(String(activePlanId));
  ...
  const enroll = async (trx) => {
    ...
    if (coveredBySubscription) {
      ...
      if (usage) {
        const currentUsageCount = Number.isFinite(Number(usage.usage_count))
