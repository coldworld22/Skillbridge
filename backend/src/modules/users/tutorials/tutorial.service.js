...
const deserializeIncludedPlans = (value) => normalizeIncludedPlans(ensureArray(value));

const hydrateTutorialRow = (row) => {
  if (!row) return row;
  return {
    ...row,
    included_plans: deserializeIncludedPlans(row.included_plans),
  };
};
  ...
  return withTransaction(async (trx) => {
    ...
    if (Array.isArray(chapters) && chapters.length) {
      const normalizedChapters = chapters.map((chapter, index) => ({
        ...
        tenant_id: tutorial.tenant_id || null,
        title: chapter.title,
...
exports.getAllTutorials = async (filters = {}) => {
  ...
  const [totalResult, rawTutorials] = await Promise.all([countQuery, dataQuery]);
  const tutorials = rawTutorials.map(hydrateTutorialRow);
...
exports.getTutorialById = async (id, userId = null) => {
  ...
  try {
    const tutorial = await buildQuery(true);
    return hydrateTutorialRow(tutorial);
  } catch (err) {
    if (isMissingViewTableError(err)) {
      logMissingViewTableOnce();
      return hydrateTutorialRow(await buildQuery(false));
    }
    throw err;
  }
};
...
exports.getTutorialsByInstructor = async (instructorId) => {
  const runQuery = async (includeViews = true) => {
    ...
    const tutorials = (await query).map(hydrateTutorialRow);
...
exports.getArchivedTutorials = async () => {
  const rows = await db("tutorials")
    .where({ status: "archived" })
    .orderBy("updated_at", "desc");
  return rows.map(hydrateTutorialRow);
};
...
exports.getFeaturedTutorials = async () => {
  ...
  const rows = await db({ t: "tutorials" })
    .leftJoin("users as u", "t.instructor_id", "u.id")
...
exports.getPublicTutorialDetails = async (id) => {
  ...
  const tutorialRow = await db({ t: "tutorials" })
    .leftJoin("users as u", "t.instructor_id", "u.id")
  ...
  const tutorial = hydrateTutorialRow(tutorialRow);
