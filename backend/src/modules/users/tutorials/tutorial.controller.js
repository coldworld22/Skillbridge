...
const unlinkAndSubtract = async (fileUrl, tenantId) => {
  if (!fileUrl) return;
  ...
  try {
    ...
    logger.warn("Failed to delete tutorial asset", err.message);
  }
};
...
exports.createTutorial = catchAsync(async (req, res) => {
  ...
  const existing = await db("tutorials")
    .modify((qb) => {
      qb.whereRaw("LOWER(title) = ?", title.toLowerCase());
...
exports.updateTutorial = catchAsync(async (req, res) => {
  ...
  const existing = await service.getTutorialById(req.params.id);
  if (!existing) {
    throw new AppError("Tutorial not found", 404);
  }
  const { tags: rawTags, chapters: _rawChapters, ...data } = req.body;
  const roleDir = getRoleDir(req);
  if (req.files?.thumbnail) {
    data.cover_image = `/uploads/tutorials/${roleDir}/${req.files.thumbnail[0].filename}`;
    await unlinkAndSubtract(existing.cover_image, tenantId || existing.tenant_id);
  }
  if (req.files?.preview) {
    data.preview_video = `/uploads/tutorials/${roleDir}/${req.files.preview[0].filename}`;
    await unlinkAndSubtract(existing.preview_video, tenantId || existing.tenant_id);
  }
  ...
  data.tenant_id = tenantId || existing.tenant_id || null;
...
exports.permanentlyDeleteTutorial = catchAsync(async (req, res) => {
  ...
  const existing = await service.getTutorialById(req.params.id);
  if (existing) {
    const tid = tenantId || existing.tenant_id;
    await unlinkAndSubtract(existing.cover_image, tid);
    await unlinkAndSubtract(existing.preview_video, tid);
    // Clean up chapter videos
    const chapters = await db("tutorial_chapters")
      .where({ tutorial_id: req.params.id })
      .select("video_url", "tenant_id");
    for (const ch of chapters) {
      await unlinkAndSubtract(ch.video_url, tid || ch.tenant_id);
    }
  }
