...
exports.deleteMessage = catchAsync(async (req, res) => {
  ...
  const tenantId = req.tenant?.id || deleted?.tenant_id || null;
  const maybeUnlink = async (filePath) => {
    if (!filePath) return;
    ...
    try {
      ...
      logger.warn("Failed to delete group message attachment", err.message);
    }
  };

  await maybeUnlink(deleted.file_url);
  await maybeUnlink(deleted.audio_url);
