...
async function mockRecordPurchase(studentId, bookId, pricePaid = 0) {
  const numericBookId = Number(bookId);
  const normalizedBookId = Number.isFinite(numericBookId) ? numericBookId : bookId;
  await mockDb('book_purchases').insert({
...
jest.mock('../../paymentConfig/paymentConfig.service', () => ({
  getSettings: jest.fn().mockResolvedValue({ platformCut: { book: 10 } }),
}));
jest.mock('../../payments/helpers/wallet', () => ({
  creditInstructorSubscription: jest.fn(),
}));
jest.mock('../../payments/payments.service', () => {
  const STATUS = { PAID: 'paid', AWAITING_APPROVAL: 'awaiting_approval' };
  const insertPayment = async (payload, trx, dbRef) => {
    const executor = trx || dbRef;
    if (typeof executor === 'function') {
      await executor('payments').insert({
        id: payload.id,
        ...
        instructor_amount: payload.instructor_amount ?? 0,
      });
    }
    return payload;
  };
  return {
    STATUS,
    create: jest.fn(async (data, _files = [], trx, dbRef = mockGetTestDb()) => {
      const payload = {
        id: data.id || 'pay-' + Math.random(),
        ...data,
        status: STATUS.AWAITING_APPROVAL,
      };
      await insertPayment(payload, trx, dbRef);
      return payload;
    }),
    approveBankPayment: jest.fn(async (id, payload = {}, dbRef = mockGetTestDb()) => {
      await dbRef('payments')
        .where({ id })
        .update({
          status: STATUS.PAID,
          paid_at: new Date(),
        });
      const updated = await dbRef('payments').where({ id }).first();
      if (updated) {
        return updated;
      }
      return {
...
jest.mock('../../library/library.service', () => ({
  recordPurchase: jest.fn((studentId, bookId, pricePaid) =>
    mockRecordPurchase(studentId, bookId, pricePaid)
  ),
}));
jest.mock('../../payments/paymentAccess', () => ({
  grantAccess: jest.fn((payment) =>
    mockRecordPurchase(payment.user_id, payment.item_id, payment.amount ?? 0)
  ),
}));
...
describe('updateBook', () => {
  test('removes old media files when new ones are uploaded', async () => {
    ...
    try {
      ...
      fs.rmSync(oldCover, { force: true });
      fs.rmSync(oldPdf, { force: true });
    }
  });
});
