...
const { enforceBaseCurrency, getMinorUnitsMultiplier } = require("./currency");
...
async function validatePaymentData(body, userId, options = {}) {
  ...
  if (method.type === "paypal") {
    ...
    const multiplier = getMinorUnitsMultiplier(verifiedCurrency);
    verifiedAmount =
      charge.amount && Number.isFinite(charge.amount)
        ? charge.amount / multiplier
        : verifiedAmount;
    ...
    if (typeof classService.getClassById === "function") {
      try {
        ...
        const allowFallback =
          ...
          process.env.SKIP_CLASS_PRICE_VALIDATION === "true";
        if (!allowFallback) {
          throw new AppError("Unable to retrieve class details", 500);
        }
        try {
          logger.warn(
            "Class lookup failed during payment validation; skipping amount enforcement for test run:",
            err.message || err
          );
        } catch (_) {
