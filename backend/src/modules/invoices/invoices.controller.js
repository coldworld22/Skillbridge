...
exports.getInvoices = catchAsync(async (req, res) => {
  const data = await service.getInvoices(req.tenant?.id);
  sendSuccess(res, data);
});

exports.getInvoice = catchAsync(async (req, res) => {
  const invoice = await service.getInvoice(req.params.id, req.tenant?.id);
  if (!invoice) throw new AppError("Invoice not found", 404);
  sendSuccess(res, invoice);
});

exports.getMyInvoices = catchAsync(async (req, res) => {
  const data = await service.getInvoicesByUser(req.user.id, req.tenant?.id);
  sendSuccess(res, data);
});
