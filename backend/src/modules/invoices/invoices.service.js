...
exports.generateFromPayment = async (payment, user) => {
  const existingRaw = await model.findByPayment(
    payment.id,
    payment.tenant_id,
  );
  ...
  if (existing) {
    saved = await model.update(
      id,
      baseRecord,
      payment.tenant_id || existing?.tenant_id,
    );
  } else {
