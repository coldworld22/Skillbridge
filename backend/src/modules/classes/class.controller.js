...
const hasPrivateClassAccess = async (user, classId) => {
  if (!user || !classId) return false;
  const rawRoles = resolveRoles(user);
  if (isAdminRole(rawRoles)) {
    return true;
  }
  const normalizedRoles = rawRoles.map((role) => normalizeRole(role));
  if (normalizedRoles.includes("instructor")) {
    const ownsClass = await db("online_classes")
      .select("id")
      .where({ id: classId, instructor_id: user.id })
      .first();
    if (ownsClass) {
      return true;
    }
...
exports.getPublicClassDetails = catchAsync(async (req, res) => {
  const { id } = req.params;
  const includeDraft = Boolean(
    req.user && (await hasPrivateClassAccess(req.user, id))
  );
  const cls = await service.getPublicClassDetails(id, { includeDraft });
  if (cls) {
    ...
    cls.views = await service.getClassViewCount(id);
  }
  sendSuccess(res, cls);
});
