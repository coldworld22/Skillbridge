...
const ensureActiveEnrollment = async (classId, studentId) => {
  ...
  if (enrollment.status === 'suspended') {
    throw new Error('Enrollment suspended pending payment');
  }
};

const calculateForStudent = async (classId, studentId) => {
  await ensureActiveEnrollment(classId, studentId);
...
const calculateForClass = async (classId) => {
  let enrollmentQuery = db('class_enrollments as ce').join('users as u', 'ce.user_id', 'u.id');

  if (typeof enrollmentQuery.where === 'function') {
    const result = enrollmentQuery.where('ce.class_id', classId);
    if (result) {
      enrollmentQuery = result;
    }
  }

  if (typeof enrollmentQuery.whereNot === 'function') {
    const result = enrollmentQuery.whereNot('ce.status', 'cancelled');
    if (result) {
      enrollmentQuery = result;
    }
  } else if (typeof enrollmentQuery.where === 'function') {
    const result = enrollmentQuery.where('ce.status', '!=', 'cancelled');
    if (result) {
      enrollmentQuery = result;
    }
  }

  const enrollments = await enrollmentQuery.select('u.id as user_id', 'u.full_name');
