...
const sanitizeMethod = (method) => {
  if (method?.settings) {
    ...
    const coinbaseSecret =
      method.settings.api_secret || method.settings.webhook_secret;
    method.settings.has_api_secret = Boolean(coinbaseSecret);
    delete method.settings.client_secret;
    delete method.settings.secret_key;
    delete method.settings.api_secret;
    delete method.settings.webhook_secret;
  }
  return method;
};
...
exports.createMethod = catchAsync(async (req, res) => {
  ...
  if ((data.type || "").toLowerCase() === "paypal") {
    ...
    data.settings = {
      ...
      mode:
        (data.settings?.mode || envMode || "sandbox")?.toLowerCase?.() ||
        "sandbox",
    };
  }
  if (req.file) {
  ...
  if (req.file) {
    if (existing.icon) {
      ...
      if (fs.existsSync(old)) {
        const oldSize = fs.statSync(old)?.size || 0;
        fs.unlinkSync(old);
...
exports.updatePayPalCredentials = catchAsync(async (req, res) => {
  ...
  if (
    normalizedMode &&
    !["sandbox", "live"].includes(normalizedMode.toLowerCase())
  ) {
