...
exports.createTicket = async ({ user_id, subject, message }) => {
  ...
  do {
    ...
    await db("support_tickets").where({ ticket_number: ticketNumber }).first()
  );
...
exports.listUserTickets = (user_id) => {
  return db("support_tickets").where({ user_id }).orderBy("created_at", "desc");
};
...
exports.findAttachmentById = (id) =>
  db("support_attachments").where({ id }).first();

exports.deleteAttachment = async (id) => {
  const existing = await exports.findAttachmentById(id);
  if (!existing) return null;
  await db("support_attachments").where({ id }).del();
  return existing;
};
...
exports.removeTicket = (id) => db("support_tickets").where({ id }).del();
...
exports.getAnalytics = async () => {
  const [openRow] = await db("support_tickets")
    .where({ status: "open" })
    .count();
