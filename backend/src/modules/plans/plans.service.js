...
const { groupResourcesByPlan } = require("./planResourceUtils");
...
exports.getPlans = async (role) => {
  ...
  const classesByPlan = groupResourcesByPlan(
    rawClasses.filter(
      (cls) =>
        (cls.status || "").toLowerCase() === "published" &&
        (cls.moderation_status || "").toLowerCase() === "approved"
    ),
    (cls) => ({
      id: cls.id,
      title: cls.title,
  ...
  const allowedBookStatuses = new Set(["approved", "active"]);
  const booksByPlan = groupResourcesByPlan(
    rawBooks.filter((book) =>
      allowedBookStatuses.has((book.status || "").toLowerCase())
    ),
    (book) => ({
      id: book.id,
      title: book.title,
  ...
  const tutorialsByPlan = groupResourcesByPlan(rawTutorials, (tutorial) => ({
    id: tutorial.id,
  ...
  return plans.map((plan) => {
    ...
    return {
      ...plan,
      features: combined,
      included_classes: sortByTitle(classesByPlan[plan.id] || []),
      included_books: sortByTitle(booksByPlan[plan.id] || []),
      included_tutorials: sortByTitle(tutorialsByPlan[plan.id] || []),
    };
  });
};
