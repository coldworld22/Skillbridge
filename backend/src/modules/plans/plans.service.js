...
exports.getPlans = async (role) => {
  ...
  return plans.map((plan) => {
    const formattedFeatures = features
      ...
      .map((feature) => {
        const storedValue = feature.value;
        const parsedValue = parseFeatureValue(storedValue);
    ...
    const syntheticFeatures = SYNTHETIC_PLAN_FEATURES.filter((def) =>
      ...
      .map((def) => {
        ...
        const rawValue =
          built.raw !== undefined && built.raw !== null ? built.raw : null;
        const displayValue =
          built.display !== undefined && built.display !== null
            ? built.display
            : built.value !== undefined && built.value !== null
              ? built.value
              : rawValue !== null
                ? String(rawValue)
                : "";
        ...
        return {
          ...
          module: def.module,
          raw_value: rawValue,
          parsed_value: built.parsed ?? rawValue,
          source: "computed",
        };
      })
      .filter(Boolean);
