...
const ONBOARDING_ALLOWED_PATTERNS = [
  ...
  /^\/api\/users\/admin\/[^/]+\/(avatar|identity)$/i,
...
const verifyToken = async (req, res, next) => {
  ...
  try {
    ...
    const memberships =
      decoded.memberships ||
      (userModel.getUserMemberships
        ? await userModel.getUserMemberships(decoded.id)
    ...
    req.user = {
      ...
      permissions,
      platform_role: decoded.platform_role || user.platform_role || "none",
      memberships,
      onboardingComplete,
    };
    next();
...
const isInstructorOrAdmin = (req, res, next) => {
  ...
  if (
    ...
    norm.some((r) => ["admin", "superadmin"].includes(r))
  ) {
    return next();
  }
