...
copy_if_missing() {
  ...
  local dest="$2"
  if [[ -f "$dest" ]]; then
    log "Keeping existing $(relpath "$dest")"
    return 0
  fi
  if [[ ! -f "$src" ]]; then
    warn "Template $(relpath "$src") missing; skipping."
    return 0
  fi
  mkdir -p "$(dirname "$dest")"
  cp "$src" "$dest"
  log "Created $(relpath "$dest") from template."
}
...
get_env_value() {
  ...
  local key="$2"
  local fallback="${3:-}"
  [[ -f "$file" ]] || { echo "$fallback"; return 0; }
  local line
  line="$(grep -E "^\s*${key}=" "$file" | tail -n1 || true)"
  if [[ -z "$line" ]]; then
    echo "$fallback"
    return 0
  fi
...
- Remember to disable INSTALL_API_ENABLED once setup is finished.
