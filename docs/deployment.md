...
- Check the API health endpoint with `curl https://<domain>/api/health` and
  confirm it returns `{"status":"ok"}`.
- Sign in to the frontend, navigate to `/dashboard/admin`, and confirm pages
  load without console errors.
- Send a password reset email or test notification to verify SMTP credentials
  are correct.
- Run `docker compose ps` (or your process manager equivalent) to confirm
  backend, frontend, and worker processes are running.
- Review `docker compose logs backend` and `docker compose logs nginx` for
  warnings about migrations, CORS, or TLS.
