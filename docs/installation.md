...
> **Heads up:** The legacy `docker-compose` v1 CLI is incompatible with recent Docker Engine releases and often fails with `KeyError: 'ContainerConfig'` when rebuilding containers. Install Docker Compose V2 and prefer the `docker compose` command. If you cannot upgrade immediately, export `DOCKER_BUILDKIT=0` and `COMPOSE_DOCKER_CLI_BUILD=0` before running Compose so the legacy CLI can work with older image metadata.
