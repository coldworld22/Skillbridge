...
Plan defaults are now curated directly in the admin dashboard, so instructors
and students can be onboarded without running any extra seed scripts. The
catalogue still expects the following feature keys to be filled out:
...
When you need a new toggle, add it to the metadata helpers and surface it in
the dashboard forms. Runtime gating still relies on `parsePlanFeatures`.
...
When you add a new premium capability, create a plan feature key, expose it in
the dashboard, and check it at the service layer before performing the action.
